...
def main():
    ...
    inducer = RuleInducer(threshold=2)  # soglia per l'induzione (dopo 2 occorrenze)
    ...
    colonne = 4
    if sintomi:
        larghezza_max = max(len(s) for s in sintomi) + 2
        ...
        print("Nessun sintomo trovato nell'ontologia.")
    print()

    while True:
        sintomi_input = input("Inserisci i sintomi osservati (separati da virgola): ").strip()
        ...
        # Esegue la diagnosi con le regole attuali
        ...
        if results:
            ...
            print("Nessuna diagnosi possibile con le regole attuali.")

        # Richiede sempre il feedback (anche se non ci sono diagnosi)
        ...
        # Aggiornamento delle probabilità (solo se abbiamo un guasto valido e regole usate)
        if correct is not None and used_rules:
            rules = update_probabilities(rules, used_rules, correct, observed)

        # Induzione: registra l'osservazione per future nuove regole
        if correct is not None:
            inducer.add_observation(observed, correct, rules)

        # Controlla se ci sono candidati che hanno superato la soglia
    ...
    # Salva le regole aggiornate su file
